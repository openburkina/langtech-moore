...
class _UpdateProfilState extends State<UpdateProfil> {
  ...
  void _getProfilInfos() {
    ...
    if (user.nom == '') {
      ...
      Toast.showFlutterToast(
          context, "Veuillez entrer votre prénom !", 'warning');
    } else if (user.telephone == '') {
      Toast.showFlutterToast(
          context, "Veuillez entrer votre numéro de téléphone !", 'warning');
    } else {
      _updateProfil();
    }
  }
  ...
  void _updateProfil() {
    ...
    Http.onUpdateProfil(user).then((response) {
      ...
      if (response.statusCode == 200) {
        Toast.showFlutterToast(
            context, "Votre profil a été modifié avec succès !", "success");
  ...
  void _saveUpdateUserInfos(String updateUser) async {
    ...
    bool savedResponse = await SharedPrefConfig.saveStringData(
        SharePrefKeys.USER_INFOS, "${updateUserInfos}");
    if (savedResponse) {
